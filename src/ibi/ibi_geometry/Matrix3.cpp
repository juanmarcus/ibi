...
Matrix3 Matrix3::operator*(const Matrix3& rkMatrix) const
{
	...
	for (size_t iRow = 0; iRow < 3; iRow++)
	{
		for (size_t iCol = 0; iCol < 3; iCol++)
		{
			kProd.m[iRow][iCol] = m[iRow][0] * rkMatrix.m[0][iCol] + m[iRow][1]
					* rkMatrix.m[1][iCol] + m[iRow][2] * rkMatrix.m[2][iCol];
		}
	}
	return kProd;
}
...
Vector3 Matrix3::operator*(const Vector3& rkPoint) const
{
	...
	for (size_t iRow = 0; iRow < 3; iRow++)
	{
		kProd[iRow] = m[iRow][0] * rkPoint[0] + m[iRow][1] * rkPoint[1]
				+ m[iRow][2] * rkPoint[2];
	}
	return kProd;
}
...
Vector3 operator*(const Vector3& rkPoint, const Matrix3& rkMatrix)
{
	...
	for (size_t iRow = 0; iRow < 3; iRow++)
	{
		kProd[iRow] = rkPoint[0] * rkMatrix.m[0][iRow] + rkPoint[1]
				* rkMatrix.m[1][iRow] + rkPoint[2] * rkMatrix.m[2][iRow];
	}
	return kProd;
}
//-----------------------------------------------------------------------
...
bool Matrix3::Inverse(Matrix3& rkInverse, Real fTolerance) const
{
	...
	Real fDet = m[0][0] * rkInverse[0][0] + m[0][1] * rkInverse[1][0] + m[0][2]
			* rkInverse[2][0];
...
Real Matrix3::Determinant() const
{
	...
	Real fDet = m[0][0] * fCofactor00 + m[0][1] * fCofactor10 + m[0][2]
			* fCofactor20;
...
void Matrix3::Bidiagonalize(Matrix3& kA, Matrix3& kL, Matrix3& kR)
{
	...
	fLength = Math::Sqrt(kA[0][0] * kA[0][0] + kA[1][0] * kA[1][0] + kA[2][0]
			* kA[2][0]);
	if (fLength > 0.0)
	{
...
void Matrix3::GolubKahanStep(Matrix3& kA, Matrix3& kL, Matrix3& kR)
{
	...
	// adjust right
	Real fY = kA[0][0]
			- (Math::Abs(fRoot1 - fT22) <= Math::Abs(fRoot2 - fT22) ? fRoot1
					: fRoot2);
	Real fZ = kA[0][1];
...
void Matrix3::SingularValueDecomposition(Matrix3& kL, Vector3& kS, Matrix3& kR) const
{
	...
	for (unsigned int i = 0; i < ms_iSvdMaxIterations; i++)
	{
		...
		bool bTest1 = (Math::Abs(kA[0][1]) <= ms_fSvdEpsilon * (Math::Abs(
				kA[0][0]) + Math::Abs(kA[1][1])));
		bool bTest2 = (Math::Abs(kA[1][2]) <= ms_fSvdEpsilon * (Math::Abs(
				kA[1][1]) + Math::Abs(kA[2][2])));
		if (bTest1)
		{
			if (bTest2)
			{
			...
			else
			{
				...
				fTmp = (kA[1][1] * kA[1][1] - kA[2][2] * kA[2][2] + kA[1][2]
						* kA[1][2]) / (kA[1][2] * kA[2][2]);
				...
				kS[0] = kA[0][0];
				kS[1] = fCos0 * fCos1 * kA[1][1] - fSin1 * (fCos0 * kA[1][2]
						- fSin0 * kA[2][2]);
				kS[2] = fSin0 * fSin1 * kA[1][1] + fCos1 * (fSin0 * kA[1][2]
						+ fCos0 * kA[2][2]);
				break;
			}
		}
		else
		{
			...
			{
				// 2x2 closed form factorization
				fTmp = (kA[0][0] * kA[0][0] + kA[1][1] * kA[1][1] - kA[0][1]
						* kA[0][1]) / (kA[0][1] * kA[1][1]);
				fTan0 = 0.5 * (-fTmp + Math::Sqrt(fTmp * fTmp + 4.0));
				fCos0 = Math::InvSqrt(1.0 + fTan0 * fTan0);
				fSin0 = fTan0 * fCos0;
				...
				kS[0] = fCos0 * fCos1 * kA[0][0] - fSin1 * (fCos0 * kA[0][1]
						- fSin0 * kA[1][1]);
				kS[1] = fSin0 * fSin1 * kA[0][0] + fCos1 * (fSin0 * kA[0][1]
						+ fCos0 * kA[1][1]);
				kS[2] = kA[2][2];
				break;
			}
...
//-----------------------------------------------------------------------
void Matrix3::SingularValueComposition(const Matrix3& kL, const Vector3& kS,
		const Matrix3& kR)
{
	size_t iRow, iCol;
	Matrix3 kTmp;
...
void Matrix3::Orthonormalize()
{
	...
	// compute q0
	Real fInvLength = Math::InvSqrt(m[0][0] * m[0][0] + m[1][0] * m[1][0]
			+ m[2][0] * m[2][0]);
	...
	fInvLength = Math::InvSqrt(m[0][1] * m[0][1] + m[1][1] * m[1][1] + m[2][1]
			* m[2][1]);
	...
	fInvLength = Math::InvSqrt(m[0][2] * m[0][2] + m[1][2] * m[1][2] + m[2][2]
			* m[2][2]);
...
Real Matrix3::SpectralNorm() const
{
	...
	afCoeff[0] = -(kP[0][0] * (kP[1][1] * kP[2][2] - kP[1][2] * kP[2][1])
			+ kP[0][1] * (kP[2][0] * kP[1][2] - kP[1][0] * kP[2][2]) + kP[0][2]
			* (kP[1][0] * kP[2][1] - kP[2][0] * kP[1][1]));
	afCoeff[1] = kP[0][0] * kP[1][1] - kP[0][1] * kP[1][0] + kP[0][0]
			* kP[2][2] - kP[0][2] * kP[2][0] + kP[1][1] * kP[2][2] - kP[1][2]
			* kP[2][1];
		...
		else
		{
			...
			if (m[0][0] >= m[1][1])
			{
				...
				if (m[0][0] >= m[2][2])
				{
					...
					rkAxis.x = 0.5 * Math::Sqrt(m[0][0] - m[1][1] - m[2][2]
							+ 1.0);
				...
				else
				{
					...
					rkAxis.z = 0.5 * Math::Sqrt(m[2][2] - m[0][0] - m[1][1]
							+ 1.0);
			...
			else
			{
				...
				if (m[1][1] >= m[2][2])
				{
					...
					rkAxis.y = 0.5 * Math::Sqrt(m[1][1] - m[0][0] - m[2][2]
							+ 1.0);
...
bool Matrix3::QLAlgorithm(Real afDiag[3], Real afSubDiag[3])
{
	...
	for (int i0 = 0; i0 < 3; i0++)
	{
		...
		for (iIter = 0; iIter < iMaxIter; iIter++)
		{
			...
			if (fTmp0 < 0.0)
				fTmp0 = afDiag[i1] - afDiag[i0] + afSubDiag[i0] / (fTmp0
						- fTmp1);
			else
				fTmp0 = afDiag[i1] - afDiag[i0] + afSubDiag[i0] / (fTmp0
						+ fTmp1);
