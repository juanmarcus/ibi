...
Quaternion Quaternion::operator*(const Quaternion& rkQ) const
{
	...
	return Quaternion(w * rkQ.w - x * rkQ.x - y * rkQ.y - z * rkQ.z, w * rkQ.x
			+ x * rkQ.w + y * rkQ.z - z * rkQ.y, w * rkQ.y + y * rkQ.w + z
			* rkQ.x - x * rkQ.z, w * rkQ.z + z * rkQ.w + x * rkQ.y - y * rkQ.x);
}
//-----------------------------------------------------------------------
...
Quaternion Quaternion::Inverse() const
{
	...
	if (fNorm > 0.0)
	{
		...
		return Quaternion(w * fInvNorm, -x * fInvNorm, -y * fInvNorm, -z
				* fInvNorm);
	}
	else
	{
...
bool Quaternion::equals(const Quaternion& rhs, const Radian& tolerance) const
{
	...
	return (Math::Abs(angle.valueRadians()) <= tolerance.valueRadians())
			|| Math::RealEqual(angle.valueRadians(), Math::PI,
...
Radian Quaternion::getRoll(bool reprojectAxis) const
{
	...
	{
		return Radian(Math::ATan2(2 * (x * y + w * z), w * w + x * x - y * y
				- z * z));
	}
}
//-----------------------------------------------------------------------
Radian Quaternion::getPitch(bool reprojectAxis) const
{
	...
	{
		// internal version
		return Radian(Math::ATan2(2 * (y * z + w * x), w * w - x * x - y * y
				+ z * z));
	}
}
//-----------------------------------------------------------------------
